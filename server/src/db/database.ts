...
export class DatabaseManager {
  ...
  /**
   * Execute an atomic transaction with immediate write locking to prevent race conditions.
   * Supports re-entrant nested transaction calls.
   */
  public transaction<T>(fn: () => T): T {
    if (this.transactionDepth > 0) {
      // Already inside an active transaction, execute directly
      return fn();
    }
